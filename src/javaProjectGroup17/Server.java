...
public class Server {
	...
	public static void main(String [] args) {
		...
		JFrame playerSettingFrame1 = new JFrame("Server"); //版本選擇的frame，單機版or雙人版
		...
		singleBtn.addActionListener(new ServerBtnListener());
		doubleBtn.addActionListener(new ServerBtnListener());
		...
		if(singleVersion == true) { //單機版
			...
			try{
				...
				if(message.equals("putMouse")) {
					ServerBtnListener.putBtn.setEnabled(false);
				}
				else if(message.equals("catchMouse")) {
					ServerBtnListener.catchBtn.setEnabled(false);
				}
				if(putMouse == true) {
				...
				if(putMouse == true) { //放老鼠
					ServerBtnListener.playerSettingFrame2.dispose();
					initialize();
					...
					while(true) { //
						if(myScore!=-1 && hisScore!=-1) { //遊戲結束，產生輸贏的frame
							if(myScore > hisScore) {
								frameForWinnerAndLoser("放地鼠Server:你贏了", player.frame, output);
							}
							else if(myScore < hisScore) {
								frameForWinnerAndLoser("放地鼠Server:你輸了", player.frame, output);
							}
							else if(myScore == hisScore) {
								frameForWinnerAndLoser("放地鼠Server:平手", player.frame, output);
							}
							myScore = hisScore = -1;
						}
						else {
					...
					while(true) { //
						if(myScore!=-1 && hisScore!=-1) { //遊戲結束，產生輸贏的frame
							if(myScore > hisScore) {
								frameForWinnerAndLoser("打地鼠Server:你贏了", player.frame, output);
							}
							else if(myScore < hisScore) {
								frameForWinnerAndLoser("打地鼠Server:你輸了", player.frame, output);
							}
							else if(myScore == hisScore) {
								frameForWinnerAndLoser("打地鼠Server:平手", player.frame, output);
							}
							myScore = hisScore = -1;
						}
						else {
	...
	private static void frameForWinnerAndLoser(String text, JFrame playerFrame, DataOutputStream output) { //有遊戲結果後，選擇要再來一次or退出遊戲的frame
		...
		restartBtn.addActionListener(new ServerBtnListener(playerSettingFrame3, playerFrame, output));
		exitBtn.addActionListener(new ServerBtnListener(playerSettingFrame3, playerFrame, output));
			...
			try {
				...
				while(true) {
					...
					if(inputStr.length()>4 && inputStr.substring(0, 5).equals("score")) { //
						Server.hisScore = Integer.parseInt(inputStr.substring(5));
						inputStr = "";
					}
					Server.message = inputStr;
				}
				input.close();
				socket.close();
				System.exit(0);
	...
	private static class ServerBtnListener implements ActionListener{ //ButtonListener
		...
		ServerBtnListener(JFrame f, JFrame player_frame, DataOutputStream o) {
			deleteFrame = f;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(command.equals("單機版")) {//singleVersion = true
				Server.singleVersion = true;
			}
			else if(command.equals("雙人版")) {//doubleVersion = true
				Server.doubleVersion = true;

				playerSettingFrame2 = new JFrame("Server"); //選擇要抓老鼠還是放老鼠
				...
				putBtn.addActionListener(new ServerBtnListener());
				catchBtn.addActionListener(new ServerBtnListener());
